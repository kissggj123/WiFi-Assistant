                     ...
                     _In_ int       nCmdShow)
{
	...
	{
		MessageBox(NULL, L"您的操作系统不支持!", L"错误", MB_OKCANCEL | MB_ICONERROR);
		return 1;
	}
#else
	OSVERSIONINFO osvi;
	...
	HANDLE hMutex = CreateMutex(NULL, FALSE, _T("虚拟WiFi"));
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		HWND hOnK = CreateWindowEx(WndStyle::dwBtEx, WC_BUTTON, L"一键启动", WndStyle::dwBt, 110, 50, 200, 30, hWnd, HMENU(IDC_BT_ONEAUTO), hInst, nullptr);
		HWND hStSSID = CreateWindowEx(WndStyle::dwStEx, WC_STATIC, L"WiFi名称:", WndStyle::dwSt, 30, 90, 70, 25, hWnd, HMENU(IDC_ST_SSID), hInst, nullptr);
		...
		HWND hSTime = CreateWindowEx(WndStyle::dwStEx, WC_STATIC, L"时间[天|小时|分钟]:", WndStyle::dwSt, 110, 250, 100, 25, hWnd, HMENU(IDC_ST_TIME), hInst, nullptr);
		...
		if (Global::IsAdmin)
		{
			...
			SetWindowText(hRunAs, L"已获得管理员权限");
			
		}
		else
		{
			SetWindowText(hRunAs, L"获取管理员权限");
			HWND hInfo = CreateWindowEx(WndStyle::dwStEx, WC_STATIC, L"虚拟WiFi需要提升权限,请点击\"获取管理员权限\"按钮", WndStyle::dwSt, 110, 365, 200, 40, hWnd, nullptr, hInst, nullptr);
	...
	case WM_COMMAND:
		...
		case IDC_BT_ONEAUTO:
		{
			if (!Global::IsAdmin)
			{
				MessageBox(hWnd, TEXT("请用管理员身份运行!"), TEXT("错误"), MB_OK | MB_ICONWARNING);
				break;
			}
			WCHAR Ermsg[512] = { 0 };
			...
			if (lr != S_OK)
			{
				...
				MessageBox(hWnd, Ermsg, L"错误", MB_YESNO | MB_ICONERROR);
			}
			else{
				SetWindowText(GetDlgItem(hWnd, IDC_EDIT_SSID), Global::wifiifo.SSID);
				SetWindowText(GetDlgItem(hWnd, IDC_LIMIT_EDIT), Global::wifiifo.KEY);
				MessageBox(hWnd, L"创建虚拟WiFi成功!", L"成功", MB_OK | MB_ICONINFORMATION);
			}
		}
			break;
		...
		case IDC_BT_CLOSE:
		{
			...
			if (lrest != S_OK)
			{
				...
				MessageBox(hWnd, Errorclose, L"错误", MB_OKCANCEL | MB_ICONERROR);
			}
			else
			{
				MessageBox(hWnd, L"虚拟WiFi已经关闭!", L"操作提示", MB_OK | MB_ICONINFORMATION);
			}
		}
			break;
		case IDC_BT_OPEN:
		{
			...
			if (hr != S_OK)
			{
				...
				MessageBox(hWnd, L"虚拟WiFi正在运行!", L"News Tip", MB_OK | MB_ICONINFORMATION);
			}
		}
		break;
